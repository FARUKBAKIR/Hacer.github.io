...
(function ($) {
  ...
  var HacerHacer = {
    init: function () {
      HacerHacer.animatedText();
      HacerHacer.imgToSVG();
      HacerHacer.BgImg();

      HacerHacer.headerTrigger();
      HacerHacer.menuFixer();
      HacerHacer.timeLine();
      HacerHacer.movingBlog();
      HacerHacer.navFixer();
      HacerHacer.anchor();
      HacerHacer.movingTextForNav();
      HacerHacer.filterItems();
      HacerHacer.applyFilter();
      HacerHacer.magnific();
      HacerHacer.progressTotop();
      HacerHacer.productModal();
      HacerHacer.contactForm();
      HacerHacer.moreCategories();
      HacerHacer.totop();
      HacerHacer.vegasSlider();
      HacerHacer.heroJarallaxSlider();
      HacerHacer.subscribe();
    },
    ...
    subscribe: function () {
      $(".subscribe_form a").on("click", function () {
        ...
        if (email === "") {
          ...
          if (HacerHacer.validateEmail(email)) {
            $.post(
              "modal/subscribe.php",
    ...
    vegasSlider: function () {
      $(".hacer_fn_hero .vegas-slide").each(function () {
        var element = $(this);
        var html = [];
    ...
    heroJarallaxSlider: function () {
      // slider
      var section = $(".hacer_fn_hero .swiper-container");
    ...
    moreCategories: function () {
      $(".hacer_fn_categories").each(function () {
        var element = $(this);
    ...
    showMore: function () {
      $(".hacer_fn_categories .more")
        ...
        .on("click", function () {
          var element = $(this),
            parent = element.closest(".hacer_fn_categories"),
    ...
    productModal: function () {
      var modal = $(".hacer_fn_modal.product_modal");
      $(".hacer_fn_drops .item a")
        .off()
        .on("click", function () {
          var element = $(this);
          ...
          modal.find(".hacer_fn_title .fn_title").text(title);
    ...
    applyFilter: function () {
      ...
      $(".hacer_fn_filters .checkbox")
        ...
        .on("click", function () {
          ...
          var element = $(this),
            // collection wrapper
            parent = element.closest(".hacer_fn_collection"),
            // filter result box
            resultBox = parent.find(".hacer_fn_result_box"),
          ...
          // if clicked item has already clicked and clicked second time
            ...
            var index = HacerFilterArray.indexOf(id);
            if (index !== -1) {
              HacerFilterArray.splice(index, 1);
            }
          }

          HacerHacer.recallGridAfterFiltering();
    ...
    recallGridAfterFiltering: function (clear) {
      ...
      if (HacerFilterArray.length === 0) {
        filters = "*";
      }
      // if there has one or more selected filters
      else {
        $.each(HacerFilterArray, function (index, value) {
          filters += ".id" + value;
        });
      }
      console.log(HacerFilterArray);
    ...
    removeFilter: function () {
      $(".hacer_fn_result_box .result_item a")
        ...
        .on("click", function () {
          var e = $(this),
            parent = e.closest(".hacer_fn_collection"),
          ...
          resultItem.remove();
          parent
            .find('.hacer_fn_filters .checkbox[data-id="' + id + '"]')
      ...
      $(".hacer_fn_result_box .clear_all")
        ...
        .on("click", function () {
          var e = $(this),
            parent = e.closest(".hacer_fn_collection"),
            filterBox = parent.find(".hacer_fn_filters"),
            resultBox = parent.find(".hacer_fn_result_box"),
            filterCount = resultBox.find(".filter_count span");
          filterCount.text(0);
          ...
          filterBox
            ...
            .find('input[type="checkbox"]')
            .prop("checked", "");
          HacerHacer.recallGridAfterFiltering("clear");
          return false;
        });
    },
    ...
    movingTextForNav: function () {
      ...
      if (overlay.length) {
        if (!$(".hacer_fn_moving_text").length) {
          $("body").append('<div class="hacer_fn_moving_text">Close</div>');
        }
        var movingBox = $(".hacer_fn_moving_text");
        overlay
    ...
    anchor: function () {
      $(".hacer_fn_down").on("click", function () {
        var e = $(this),
          href = e.attr("href");
        if (
    ...
    navFixer: function () {
      var footerHeight = $(".hacer_fn_nav .nav_footer").outerHeight();
      $(".hacer_fn_nav .nav_content").css({
        height: $(window).height() - footerHeight + "px",
      });
    },

    movingBlog: function () {
      var blog = $(".hacer_fn_moving_blog");
      if (blog.length) {
        if (!$(".hacer_fn_moving_box").length) {
          $("body").append('<div class="hacer_fn_moving_box">');
        }
        var movingBox = $(".hacer_fn_moving_box");
        var list = $(".hacer_fn_moving_blog .item");
        list
          .on("mouseenter", function (event) {
            var element = $(this);
    ...
    timeLine: function () {
      var allContentItems = $(".hacer_fn_timeline .timeline_item");
      var progressLine = $(".hacer_fn_timeline .progress_line");
      var allProgressItems = $(".hacer_fn_timeline .progress_line li");
      var activeProgressItem = $(".hacer_fn_timeline .progress_line .active");
      var activeLine = $(".hacer_fn_timeline .progress_line .active_line");
      var allProgressButtons = $(".hacer_fn_timeline .progress_line a");
      var initialSpace = 110;
      var spaceBetweenItems = 230;

      HacerHacer.timelineClasses(allContentItems, allProgressItems, "initial");
      ...
      allProgressButtons.off().on("click", function () {
        ...
        if (!p.hasClass("active")) {
          var timeline = e.closest(".hacer_fn_timeline");
          var activeIndex = p.data("index");
          HacerHacer.timelineClasses(
            allContentItems,
            allProgressItems,
            activeIndex
          );
      ...
      var nextActive = true;
      var prevActive = true;
      var prevButton = $(".hacer_fn_timeline .nav_prev");
      var nextButton = $(".hacer_fn_timeline .nav_next");
    ...
    timelineClasses: function (allContentItems, allProgressItems, activeIndex) {
      ...
      isActive = false;
      if (activeIndex === "initial") {
        extraActiveIndex = $(".hacer_fn_timeline .progress_line .active").data(
          "index"
        );
      } else {
    ...
    headerTrigger: function () {
      var trigger = $(".hacer_fn_header .trigger");
      var navOverlay = $(".nav_overlay");
      var rightNav = $(".hacer_fn_nav");
      var trigger2 = rightNav.find(".trigger");
      var links = $(".hacer_fn_nav .nav_menu a");
      var buttons = $(".hacer_fn_nav .nav_buttons");
      var footer = $(".hacer_fn_nav .nav_footer");

      var menuItems = $(".hacer_fn_nav .nav_menu > ul > li");
